...
| Team capacity | Отдельный снимок по ролям (SO, SoE, DPM, WFM, QM, SE) + слоты + gates | `team_capacity.py` |
...
| 7a | Тело: team capacity | `build_live_capacity_markdown` | Несколько сообщений Slack **подряд** в треде: первое — **overview** (`*TEAM CAPACITY — OVERVIEW*`, slim pool / возможные команды, PR-5); далее один или несколько постов **BY ROLE** (`*TEAM CAPACITY — BY ROLE*` …), полный обзор по ролям (PR-6: все подтверждённые роли, саб-бэнды FREE→…→Restricted). Длинный BY ROLE режется на части под лимит поста; продолжения начинаются с `_… continued below._`; onboarding — в хвосте последнего chunk при необходимости. Размер chunk: `STAFFING_TEAM_CAPACITY_CHUNK_CHARS` (по умолчанию 11000). Databricks: `capacity.sql`, `project_staffing.sql` | `Occupation query failed` + текст CLI; `no_profile` / `no_sql` — короткое сообщение без таблиц |
...
| 9 | Постинг | `chat_postMessage` | Обычно одно сообщение в треде, длина ≤ ~12k на пост (усечение с пометкой в `slack_app.py`). **Team capacity:** несколько последовательных постов в том же треде; каждый пост остаётся под лимитом за счёт чанкинга BY ROLE | Ошибка Slack API в логах |
...
| Тело | Заголовки `*TEAM CAPACITY — OVERVIEW*` и `*TEAM CAPACITY — BY ROLE*` (BY ROLE может занимать несколько сообщений подряд); все роли, Primary/Alternate, слоты; при наличии snapshot — строки про `project_staffing` / Hold |
...
| **Ожидание** | Блоки **TEAM CAPACITY — OVERVIEW** и **TEAM CAPACITY — BY ROLE** (mrkdwn bold); роли SO / SoE / DPM / WFM / QM / SE; у каждой секции **Primary** и **Alternate**; оценка слотов Tier 1–4; при наличии SQL — **project_staffing** gates по tier на роль |
| **Не ожидать** | Сообщение «Tier required» / «нужен tier» для этого запроса |
| **Проверка** | Databricks профиль валиден; `sql/occupation.sql` не пустой; в ответе есть `TEAM CAPACITY` (overview/by role) и `Primary` |
...
| **Логика** | Промпт просит гипотезу tier при обоснованности (6 измерений, anti-size) и **`confidence` ≥ 0.7**; иначе tier остаётся null. Пост-хинт `apply_deal_feed_availability_tier_hint` снят (CR-5). При падении Phase B по-прежнему `anthropic_fallback` без tier. |
| **Ожидание** | Не застревать в низком confidence: модель или ставит уверенный tier, или честный null + capacity/S1 при маршрутизации |
| **Проверка** | Юнит-тесты `tests/test_extraction.py`, `tests/test_tier_classification_cr5.py` |
...
| `STAFFING_TEAM_CAPACITY_CHUNK_CHARS` | Опционально: целевой размер одного Slack-поста для детального блока BY ROLE (по умолчанию 11000, clamp 400–39000). Меньше — больше последовательных сообщений в треде, без усечения на ~12k |
...
1. **S1:** `Team capacity @who_is_available` → overview + BY ROLE (возможно несколько постов), Primary/Alternate, без «нужен tier».
