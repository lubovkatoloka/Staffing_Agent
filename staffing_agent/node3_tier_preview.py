"""
Tier → `project_role` filter for the Node 3 capacity *table preview* only.
...
# Normalized project_role values from Capacity SQL (staffing pool roles).
...
def occupation_preview_roles(tier: Optional[int]) -> Optional[frozenset[str]]:
    """
    Roles to include in the capacity preview. None = show all rows (no tier or unknown tier).
    """
    if tier is None:
        return None
...
def occupation_preview_caption_suffix(tier: Optional[int], *, max_shown: int = 15) -> str:
    ...
    if roles is None:
        return f"show {max_shown}, sorted by capacity_used ↑"
    parts = ", ".join(sorted(roles))
    return f"Tier {tier} preview — roles [{parts}] only; show {max_shown}, sorted by capacity_used ↑"
